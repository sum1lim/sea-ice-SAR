...
for file in mixed CAE_spk CAE backscatter; do
...
boxplot --result-dir ./results/NN_type_class/ --model-type classification --methods mixed CAE_spk CAE backscatter
...
for parent_dir in FYI_MYI; do 
    ...
    boxplot --result-dir ./results/NN_rms_regr/"$parent_dir" --model-type regression --methods mixed CAE_spk CAE backscatter
done
...
for parent_dir in FYI_MYI; do 
    ...
    boxplot --result-dir ./results/NN_thk_regr/"$parent_dir" --model-type regression --methods mixed CAE_spk CAE backscatter
done
